...
import { matchTrigger } from '../../triggers/matcher.js';
import { canExecute } from '../../permissions/checker.js';
import { buildVariableContext } from '../../variables/resolver.js';
import { executeActions } from '../../actions/index.js';
import { resolveGuildTenant } from '../../supabase/index.js';
...
async function sendConsolidatedReply(message, content, tableText) {
    ...
    const options = { content: content || null, components: [feedbackRow] };
...
async function handleAiMessage(message, client, config, logger, services, tenantId) {
    ...
    if (!question && !imageAttachment) {
        await message.reply('Olá! Meu nome é Gui. Em que posso ajudar?');
        return;
    }
    ...
    try {
        ...
        const wikiContextText = wikiContext ? await wikiContext.getContext(tenantId) : '';

        const result = await generateSolution({
            problemDescription: question,
            imageDataUri: imageDataUri || undefined,
            wikiContext: wikiContextText,
            userProfileContext,
            userGoalsContext,
...
export async function execute(message) {
    ...
    const guildConfig = services.tenantConfig?.get(guildId);

    if (guildConfig && guildConfig.enabled !== false) {
        const commands = guildConfig.custom_commands || [];
        const prefix = guildConfig.prefix || '!';

        let matched = false;

        for (const cmd of commands) {
            if (!canExecute(cmd, message)) continue;

            const cooldowns = services.cooldowns;
            if (cmd.cooldown > 0 && cooldowns) {
                if (cooldowns.has(guildId, message.author.id, cmd.id)) {
                    continue;
                }
            }

            if (matchTrigger(message, { ...cmd, prefix }, client)) {
                matched = true;

                if (cmd.cooldown > 0 && cooldowns) {
                    cooldowns.set(guildId, message.author.id, cmd.id, cmd.cooldown);
                }

                const ctx = buildVariableContext(message, client, guildConfig, { tenantId });
                const context = { message, ctx, client, guildConfig, tenantId };

                await executeActions(cmd.actions || [], context, cmd.executionMode || 'sequential');
    ...
    if (message.mentions.has(client.user.id) && !message.mentions.everyone) {
        await handleAiMessage(message, client, config, logger, services, tenantId);
    }
}
